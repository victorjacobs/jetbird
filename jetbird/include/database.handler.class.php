	...
	class database_handler {
		...
		public function connect($user, $pass, $db, $host = "localhost"){
			...
			if($this->link_identifier){
				...
				if(!$select) trigger_error("Database <b>$db</b> doesn't exist", E_USER_ERROR);
		...
		public function export_structure($file){
			...
			// Build output
			$output = '-- Database structure of '. $this->database .' generated by Jetbird\'s database_handler class on '. date("r") .' --'. "\n\n";
		...
		public function import($file){
			...
			// Note: our SQL file that contains only the database structure, database
			//  creation should be handled by the user
			if(!$this->connected){
				trigger_error("database_handler::import() not allowed", E_USER_ERROR);
			}
			
			// Load data
			if(!($sql_data = @file_get_contents($file))){
				// Throw error
				trigger_error("SQL file could not be loaded", E_USER_ERROR);
				die();
			}
